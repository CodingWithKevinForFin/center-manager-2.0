...
from com.f1.ami.parsed_message import ParsedMessage
...
class Receiver:
    ...
    def __init__(self):
        ...
        self._rt_receiver_cv = threading.Condition()
        self._receiver_cv = threading.Condition()
    ...
    def parse_msg(msg: str) -> ParsedMessage:
        ...
        message = message[3: len(message)-1]
    ...
    def _rt_receiver(self):
        ...
        while self._should_run:
            try:
                with self._rt_receiver_cv:
                    data = self._rt_socket.recv(2048)
                    
                    if len(data) != 0:
                        # checking the encoding for decoding to string
                    ...
                    if '\n' in self._rt_accum_string:
                        ...
                        nl = self._rt_accum_string.rfind("\n")
                        msgs = self._rt_accum_string[:nl].split("\n")
                        for callback in self._receiver_callbacks.values():
                            for msg in msgs:
                                callback(msg)
                        self._rt_accum_string = self._rt_accum_string[nl:].lstrip()
    ...
    def _logfile_parser(self):
        ...
        Receiver.logger.info(f"Monitoring logfile at [{self._logfile}]...")
    ...
    # to receive and run callbacks on the _receiver and _rt_receiver functions
    def receive(self):

        with self._rt_receiver_cv:
            self._rt_receiver_cv.notify()
        with self._receiver_cv:
            self._receiver_cv.notify()
