...
final public class AnvilServices implements AmiServicePlugin {
	...
	@Override
	public void startup(AmiImdb imdb, AmiImdbSession session, StackFrame sf, byte status) {
		if (status == STATE_STEP2_TABLES_INITIALIZED) {
			...
			schema.statsTable.insertAmiRow(schema.statsTable.createAmiPreparedRow(), session, sf);
	...
	private static final Logger log = LH.get(AnvilServices.class);
	...
	private StringBuilder getStats(StringBuilder sb, long nowNannos, AmiImdbSession session, StackFrame sf) {
		...
		schema.statsTable.updateAmiRow(row.getAmiId(), preparedStatsRow, session, sf);
	...
	public void addAlert(String type, String parentOrderId, String sector, String industry, String account, String symbol, String system, long exTime, long severity,
		...
		schema.childAlertsTable.insertAmiRow(childAlertPreparedRow, session, sf);
