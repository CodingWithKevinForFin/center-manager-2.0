...
public class DropCopyMainTest {
	...
	public static void setup() throws ConfigError, FieldConvertError, InterruptedException {
		...
		bs.setConfigDirProperty("./src/test/config/dropCopy");
		bs.startup();
	...
	public void testNewOrderMessageInput() throws ConfigError, FieldConvertError, InvalidMessage {
		...
		SAMPLE_MSG.put(TAG_TransactTime, dateFormatter.format(new Date()));
		...
		SAMPLE_MSG.put(TAG_ClOrdID, "1112");
		SAMPLE_MSG.put(TAG_OrderQty, "102");
		SAMPLE_MSG.put(TAG_TransactTime, dateFormatter.format(new Date()));
		msg = buildMessage((Map) CH.m(TAG_MsgType, MSG_TYPE_ORDER_SINGLE), SAMPLE_MSG);
		CSIM_CONTEXT.getSenderFixSession().getSenderSession().send(msg);
	...
	public void testNewOrderAckMessageReply() throws InvalidMessage {
		...
		SAMPLE_MSG.put(TAG_AvgPx, "100.0");
		SAMPLE_MSG.put(TAG_CumQty, "2");
		SAMPLE_MSG.put(TAG_LeaveQty, "99");
	...
	private static int TAG_LeaveQty = 151;
	private static int TAG_CumQty = 14;
	private static int TAG_AvgPx = 6;
