...
import com.vortex.agent.osadapter.linux.VortexAgentLinuxOsAdapterCommandRunner.ProcessWaiter;
import com.vortex.agent.state.VortexAgentOsAdapterState;

public class VortexAgentMacOsAdapterCommandRunner implements VortexAgentOsAdapterCommandRunner {
	final private static Logger log = LH.get(VortexAgentLinuxOsAdapterCommandRunner.class);
	@Override
	public VortexAgentRunOsCommandResponse runCommand(VortexAgentRunOsCommandRequest req, VortexAgentOsAdapterState state) {
		...
		if (maxRuntime <= 0) {
			r.setOk(false);
			r.setMessage("maxRuntime invalid: " + maxRuntime);
			return r;
		}
		...
		LH.info(log, "Running os command: ", command);
		r.setCommand(command);
		r.setOwner(owner);
		r.setStdin(stdin);
		try {
			if (SH.is(req.getPwd()))
				process = Runtime.getRuntime().exec(SH.splitContinous(' ',command), OH.noNull(req.getEnvVars(), OH.EMPTY_STRING_ARRAY), new File(req.getPwd()));
			else
				process = Runtime.getRuntime().exec(SH.splitContinous(' ',command));
		...
		if (completed) {
			...
			r.setOk(true);
			LH.w(log, "Command response : ", r.getStdout() );
		} else {
