...
public class LarkinPointFileLoaderProcessor extends BasicProcessor<LoadFileMessage, LarkinPointState> {

	private File basePath;
	private boolean skipFirstRow;

	public LarkinPointFileLoaderProcessor() {
			...
			try {
				...
				for (int i = 0; i < lines.length - 1; i++) {
					...
					try {
						...
						dbservice.execute("insert_daily_option_data", params, connection);
					} catch (Exception e) {
	...
	public void init() {
		...
		skipFirstRow = getServices().getPropertyController().getOptional("options.skipfirstrow", true);
