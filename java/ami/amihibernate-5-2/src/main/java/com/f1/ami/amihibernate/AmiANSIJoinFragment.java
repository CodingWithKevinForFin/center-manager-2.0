...
public class AmiANSIJoinFragment extends ANSIJoinFragment {
	 ...
	 * @param alias
	 *            The alias applied to the table being joined.
	 * @param fkColumns
	 *            The columns (from the table being joined) used to define the join-restriction (the ON)
	 * @param pkColumns
	 *            The columns (from the table being joined to) used to define the join-restriction (the ON)
	 * @param joinType
	 ...
	 *
	 * @param rhsTableName
	 *            The name of the table being joined (the RHS table).
	 * @param rhsAlias
	 *            The alias applied to the table being joined (the alias for the RHS table).
	 * @param lhsColumns
	 *            The columns (from the table being joined) used to define the join-restriction (the ON). These are the LHS columns, and are expected to be qualified.
	 * @param rhsColumns
	 *            The columns (from the table being joined to) used to define the join-restriction (the ON). These are the RHS columns and are expected to *not* be qualified.
	 * @param joinType
	 *            The type of join to produce (INNER, etc).
	 * @param on
	...
	public void addJoin(String rhsTableName, String rhsAlias, String[] lhsColumns, String[] rhsColumns, JoinType joinType, String on) {
		...
		this.buffer.append(joinString).append(rhsTableName);
		if (SH.is(rhsAlias))
			this.buffer.append(' ').append("as").append(' ').append(rhsAlias);
	...
	public JoinFragment copy() {
		final AmiANSIJoinFragment copy = new AmiANSIJoinFragment();
		copy.buffer = new StringBuilder(this.buffer.toString());
		return copy;
	}

	/**
	 * Adds a condition to the join fragment. For each given column a predicate is built in the form: {@code [alias.[column] = [condition]}
	 *
	 * @param alias
	 *            The alias to apply to column(s)
	 * @param columns
	 ...
	 *
	 * @param fromFragmentString
	 *            The join fragment string
	 */
