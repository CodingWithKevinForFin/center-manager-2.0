...
public class AmiSchemaProc_REMOVE_DATASOURCE extends AmiAbstractStoredProc {

	private static final String TYPE = "__SYSTEM";
	private static final String NAME = "__REMOVE_DATASOURCE";
	...
	public AmiSchemaProc_REMOVE_DATASOURCE(AmiImdbImpl imdb, CalcFrameStack sf) {
		this.imdb = imdb;
	...
	public FlowControl execute(AmiStoredProcRequest request, CalcFrameStack sf) throws Exception {
		...
		DatasourceRow existing = imdb.getSystemSchema().__DATASOURCE.fastLookup.getByName(name);
		if (existing == null)
			throw new RuntimeException("DATASOURCE not found: '" + name + "'");
		imdb.getSystemSchema().__DATASOURCE.removeDatasource(existing.getAmiId(), sf);
		return null;
	}
