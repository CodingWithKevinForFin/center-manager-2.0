...
public abstract class AmiCenterResultProcessor<A extends Message> extends BasicResultProcessor<A, AmiCenterState> implements AmiCenterProcessor<ResultMessage<A>, AmiCenterState> {
	...
	private ResultOutputPort<A> loopback;

	public AmiCenterResultProcessor(Class<A> type) {
		super(type, AmiCenterState.class);
		loopback = newResultOutputPort(type);
	}
	...
	public String getProcessUidFromAgentResponse(ResultMessage<?> result) {
		AmiRelayRequest req = (AmiRelayRequest) result.getRequestMessage().getAction();
