...
public class AmiKeyGenerator_FloatRand implements AmiKeyGenerator {
	private static SecureRandom RAND = MH.RANDOM_SECURE;

	private AmiColumn col;
	private AmiIndexImpl index;

	public AmiKeyGenerator_FloatRand(AmiColumnImpl col, AmiIndexImpl i) {
		OH.assertEq(col.getAmiType(), AmiTable.TYPE_FLOAT);
		this.col = col;
		this.index = i;
	}
