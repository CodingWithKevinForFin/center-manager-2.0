...
public class AmiPreparedCell_UTCN extends AmiPreparedCell {
	private long value;
	boolean isNull = false;

	public AmiPreparedCell_UTCN(AmiColumn column, boolean includeReserved) {
	...
	public void setComparable(Comparable value) {
		if (value == null)
			...
			this.value = ((DateMillis) value).getDate() * 1000000L;
		else if (value instanceof DateNanos)
			this.value = ((DateNanos) value).getTimeNanos();
		else
	...
	public Comparable getComparable() {
		return isNull ? null : new DateNanos(value);
	}
	@Override
