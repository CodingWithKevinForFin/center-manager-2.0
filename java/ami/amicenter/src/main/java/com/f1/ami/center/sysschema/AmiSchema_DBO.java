...
public class AmiSchema_DBO {

	final public AmiTableImpl table;
	final public AmiPreparedRow preparedRow;
	final public AmiColumnWrapper dboName;
	final public AmiColumnWrapper dboType;
	...
	public AmiSchema_DBO(AmiImdbImpl imdb, CalcFrameStack sf) {
		AmiTableDef def = new AmiTableDef(AmiTableUtils.DEFTYPE_SYSTEM, AmiConsts.TYPE_DBO);

		this.dboName = def.addColumn("DboName", AmiTable.TYPE_STRING, AmiConsts.NONULL_OPTIONS);
		this.dboType = def.addColumn("DboType", AmiTable.TYPE_STRING, AmiConsts.NONULL_OPTIONS);
		...
		this.table.addIndex(AmiTableUtils.DEFTYPE_SYSTEM, "pk", CH.l(dboName.getName()), CH.l(AmiIndexImpl.TYPE_SORT), AmiIndex.CONSTRAINT_TYPE_PRIMARY, null, sf);
	}
	public void addRow(AmiRow existing, String dboName, String dboType, int priority, String options, byte defType, boolean enabled, CalcFrameStack sf) {
		this.preparedRow.reset();
		this.preparedRow.setString(this.dboName, dboName);
		this.preparedRow.setString(this.dboType, dboType);
