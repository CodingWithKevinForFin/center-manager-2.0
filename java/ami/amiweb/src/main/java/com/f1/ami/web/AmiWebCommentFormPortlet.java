...
public class AmiWebCommentFormPortlet extends GridPortlet implements FormPortletListener {
	...
	private List<? extends AmiCenterRequest> reqs;
	...
	public AmiWebCommentFormPortlet(PortletConfig config, String parentPortletId, AmiCenterRequest req, String buttonName, String imageUrl) {
		this(config, parentPortletId, req == null ? Collections.EMPTY_LIST : CH.l(req), buttonName, imageUrl);
	}
	public AmiWebCommentFormPortlet(PortletConfig config, String parentPortletId, List<? extends AmiCenterRequest> req, String buttonName, String imageUrl) {
		super(config);
		...
		this.commentFormPortlet.getFormPortletStyle().setLabelsWidth(0);
		...
		if (buttonName != null) {
			submitButton = commentFormPortlet.addButton(
					new FormPortletButton(req.size() == 1 ? buttonName : (buttonName + " on " + (req.size() > 0 ? SH.toString(req.size()) + " " : "") + "selected items")));
		} else
			submitButton = null;
	...
	public AmiWebCommentFormPortlet setIconToEdit() {
		setIcon(ICON_EDIT);
		return this;
	}
	public AmiWebCommentFormPortlet setIconToCopy() {
		setIcon(ICON_COPY);
		return this;
	}
	public AmiWebCommentFormPortlet setIconToAdd() {
		setIcon(ICON_ADD);
		return this;
	}
	public AmiWebCommentFormPortlet setIconToDelete() {
		setIcon(ICON_DELETE);
		return this;
	}
