...
public class DateFieldEditFormPortlet extends BaseEditFieldPortlet<DateQueryField> implements FormPortletListener {
	...
	public DateFieldEditFormPortlet(AmiWebFormDateFieldFactory factory, AmiWebQueryFormPortlet queryFormPortlet, int fieldX, int fieldY) {
		...
		enableLastNDaysField.setLeftTopWidthHeightPx(BaseEditFieldPortlet.COL1_HORIZONTAL_POS_PX, 290, FormPortletField.DEFAULT_WIDTH, FormPortletField.DEFAULT_HEIGHT);
		...
		disableFutureDatesField.setLeftTopWidthHeightPx(BaseEditFieldPortlet.COL1_HORIZONTAL_POS_PX, 320, FormPortletCheckboxField.DEFAULT_DIM,
				FormPortletCheckboxField.DEFAULT_DIM);
	}

	@Override
	public void readFromField(DateQueryField field) {
	...
	@Override
	public void writeToField(DateQueryField field) {
		...
		Tuple2<Boolean, Boolean> t2 = new Tuple2<Boolean, Boolean>(this.disableFutureDatesField.getValue(), null);
	...
	public void onFieldValueChanged(FormPortlet portlet, FormPortletField<?> field, Map<String, String> attributes) {
		if (getHorizSettingsToggle() == field) {
			...
			if (tbf.getValue() == BaseEditFieldPortlet.FIELD_ALIGN_ADVANCED) {
				enableLastNDaysField.setLeftTopWidthHeightPx(BaseEditFieldPortlet.COL1_HORIZONTAL_POS_PX, 390, FormPortletField.DEFAULT_WIDTH, FormPortletField.DEFAULT_HEIGHT);
				disableFutureDatesField.setLeftTopWidthHeightPx(BaseEditFieldPortlet.COL1_HORIZONTAL_POS_PX, 420, FormPortletCheckboxField.DEFAULT_DIM,
						FormPortletCheckboxField.DEFAULT_DIM);
			} else {
				enableLastNDaysField.setLeftTopWidthHeightPx(BaseEditFieldPortlet.COL1_HORIZONTAL_POS_PX, 290, FormPortletField.DEFAULT_WIDTH, FormPortletField.DEFAULT_HEIGHT);
				disableFutureDatesField.setLeftTopWidthHeightPx(BaseEditFieldPortlet.COL1_HORIZONTAL_POS_PX, 320, FormPortletCheckboxField.DEFAULT_DIM,
	...
	public boolean submit() {
		...
		try {
			if (SH.is(enableLastNDays) && SH.parseInt(enableLastNDays) < 0) {
				getManager().showAlert("<b>Enable Last N Days </b> must be a positive integer");
				return false;
			} else {

				Tuple2<String, String> t = new Tuple2<String, String>(this.enableLastNDaysField.getValue(), "");
		...
		Tuple2<String, String> t = new Tuple2<String, String>(this.enableLastNDaysField.getValue(), "");
		this.queryField.getField().setEnableLastNDays(t);
		Tuple2<Boolean, Boolean> t1 = new Tuple2<Boolean, Boolean>(this.disableFutureDatesField.getValue(), false);
