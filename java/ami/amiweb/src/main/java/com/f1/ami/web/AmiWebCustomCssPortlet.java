...
public class AmiWebCustomCssPortlet extends GridPortlet
	...
	private Tab tab;
	private AmiWebCss css;

	public AmiWebCustomCssPortlet(PortletConfig config, AmiWebDesktopPortlet desktop, String layoutAlias) {
		super(config);
		this.layoutAlias = layoutAlias;
		this.cssManager = desktop.getService().getCustomCssManager();
		this.css = desktop.getService().getLayoutFilesManager().getLayoutByFullAlias(layoutAlias).getCss();
		this.origCustomCss = css.getCustomCss();
		...
		this.titleField = bodyForm.addField(new FormPortletTitleField("Custom CSS:" + (isReadonly ? " (READONLY LAYOUT)" : "")));
	...
	private void setCode(String script) {
		...
		try {
			...
			this.div.setOffset(1);
			this.css.setCustomCss(script, debugManager);
	...
	private void updatePendingChanges() {
		if (hasPendingChanges != OH.ne(this.bodyField.getValue(), css.getCustomCss())) {
	...
	public WebMenu createMenu(FormPortlet formPortlet, FormPortletField<?> field, int cursorPosition) {
		...
		AmiWebMenuUtils.createColorsMenu(r, false, this.service.getDesktop().getStylePeer());
