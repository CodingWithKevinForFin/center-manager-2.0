...
public class AmiDerivedCellCalculatorSql_UseInsert extends DerivedCellCalculatorSql {

	private String[] targetColumnNames;
	private String targetTableName;
	private DerivedCellCalculatorSql values;
	private Map<String, DerivedCellCalculator> use;

	public AmiDerivedCellCalculatorSql_UseInsert(ContainerTools tools, SqlDerivedCellParser dcp, SqlNode node, Map<String, DerivedCellCalculator> use, String[] targetColumnNames,
			String targetTableName, DerivedCellCalculatorSql values) {
		super(node, dcp);
		this.use = use;
	...
	final public FlowControl get(CalcFrameStack sf) {
		FlowControl o = values.get(sf);
		return get(sf, o);
	}

	private FlowControl get(CalcFrameStack sf, FlowControl o) {
		Table table;
	...
	public boolean isSame(DerivedCellCalculator other) {
		if (!super.isSame(other))
			return false;
		AmiDerivedCellCalculatorSql_UseInsert o = (AmiDerivedCellCalculatorSql_UseInsert) other;
