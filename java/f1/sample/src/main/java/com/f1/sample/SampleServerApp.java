...
public class SampleServerApp {
	...
	public static void main(String... args) {
		ContainerBootstrap cam = new ContainerBootstrap(SampleServerApp.class, args);
		...
		cam.setAutocodedDirectoryProperty(".sampleserver_autocoded");
		...
		cam.setLogLevel(Level.FINE, Level.CONFIG, SampleServerApp.class);
		...
		SimpleReplyProcessor replyProcessor = new SimpleReplyProcessor();
		...
		connection.addTopic(new MsgDirectTopicConfiguration("channel1", 4567, "channel1"));
		connection.addTopic(new MsgDirectTopicConfiguration("channel2", 4567, "channel2"));
		// connection.getInputTopic("channel1").subscribe(msgSuite.getEventListener());
		MsgSuite msgSuite = new MsgSuite("partition1", connection, "channel1", "channel2", null);
		rootSuite.addChildren(msgSuite, replyProcessor);
		ContainerHelper.wireCast(rootSuite, msgSuite.getInboundOutputPort(), replyProcessor, true);
	...
	public static class SimpleReplyProcessor extends BasicRequestProcessor<TextMessage, State, TextMessage> {

		public SimpleReplyProcessor() {
			super(TextMessage.class, State.class, TextMessage.class);
			bindToPartition("reply_partition");
		}

		@Override
		protected TextMessage processRequest(RequestMessage<TextMessage> action, State state, ThreadScope threadScope) {
			System.out.println("!!!!!!!!!!!!!MESSAGE: " + action);
			TextMessage r = nw(TextMessage.class);
			r.setText("replying to: " + action.getAction().getText());
			return r;
		}
