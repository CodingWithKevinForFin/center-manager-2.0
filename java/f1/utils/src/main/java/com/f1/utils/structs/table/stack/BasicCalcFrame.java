...
public class BasicCalcFrame implements CalcFrame, ToStringable {

	final private HasherMap<String, Object> values = new HasherMap<String, Object>();
	final private CalcTypes types;

	public BasicCalcFrame(CalcTypes types) {
		OH.assertNotNull(types);
		this.types = types;
	}
	...
	public void putAll(CalcFrame frame) {
		for (String i : frame.getVarKeys())
			putValue(i, frame.getValue(i));
	}
