...
package com.f1.utils.fputils;

/**
 * The class {@code FpUtils} contains static utility methods for manipulating and inspecting {@code float} and {@code double} floating-point numbers. These methods include
 * functionality recommended or required by the IEEE 754 floating-point standard.
 *
...
public class FpUtils {
	...
	/**
	 * Returns the first floating-point argument with the sign of the second floating-point argument. Note that unlike the {@link FpUtils#copySign(double, double) copySign} method,
	 * this method does not require NaN {@code sign} arguments to be treated as positive values; implementations are permitted to treat some NaN arguments as positive and other NaN
	...
	/**
	 * Returns the first floating-point argument with the sign of the second floating-point argument. Note that unlike the {@link FpUtils#copySign(float, float) copySign} method,
	 * this method does not require NaN {@code sign} arguments to be treated as positive values; implementations are permitted to treat some NaN arguments as positive and other NaN
	 * arguments as negative to allow greater performance.
	 *
	 * @param magnitude
	...
	// Commented to build on Java 1.6
	//	/**
	 ...
	 *
	 * <p>
	 * Note that this method is equivalent to the {@link Float#isInfinite(float) Float.isInfinite} method; the functionality is included in this class for convenience.
	 *
	 * @param f
	 *            the value to be tested.
	 ...
	 *
	 * <p>
	 * Note that this method is equivalent to the {@link Float#isNaN(float) Float.isNaN} method; the functionality is included in this class for convenience.
	 *
	 * @param f
	 *            the value to be tested.
	...
	/**
	 * Returns {@code true} if the unordered relation holds between the two arguments. When two floating-point values are unordered, one value is neither less than, equal to, nor
	 * greater than the other. For the unordered relation to be true, at least one argument must be a {@code NaN}.
	 *
	 * @param arg1
	 *            the first argument
	 * @param arg2
	...
	/**
	 * Returns unbiased exponent of a {@code double}; for subnormal values, the number is treated as if it were normalized. That is for all finite, non-zero, positive numbers
	 * <i>x</i>, <code>scalb(<i>x</i>, -ilogb(<i>x</i>))</code> is always in the range [1, 2).
	 * <p>
	 * Special cases:
	 * <ul>
	...
	/**
	 * Returns unbiased exponent of a {@code float}; for subnormal values, the number is treated as if it were normalized. That is for all finite, non-zero, positive numbers
	 * <i>x</i>, <code>scalb(<i>x</i>, -ilogb(<i>x</i>))</code> is always in the range [1, 2).
	 * <p>
	 * Special cases:
	 * <ul>
	...
	/**
	 * Return {@code d} &times; 2<sup>{@code scale_factor}</sup> rounded as if performed by a single correctly rounded floating-point multiply to a member of the double value set.
	 * See section 4.2.3 of <cite>The Java&trade; Language Specification</cite> for a discussion of floating-point value sets. If the exponent of the result is between the
	 * {@code double}'s minimum exponent and maximum exponent, the answer is calculated exactly. If the exponent of the result would be larger than {@code doubles}'s maximum
	 * exponent, an infinity is returned. Note that if the result is subnormal, precision may be lost; that is, when {@code scalb(x,
	 * n)} is subnormal, {@code scalb(scalb(x, n), -n)} may not equal <i>x</i>. When the result is non-NaN, the result has the same sign as {@code d}.
	 *
	 * <p>
	 * Special cases:
	 * <ul>
	...
	/**
	 * Return {@code f} &times; 2<sup>{@code scale_factor}</sup> rounded as if performed by a single correctly rounded floating-point multiply to a member of the float value set.
	 * See section 4.2.3 of <cite>The Java&trade; Language Specification</cite> for a discussion of floating-point value sets. If the exponent of the result is between the
	 * {@code float}'s minimum exponent and maximum exponent, the answer is calculated exactly. If the exponent of the result would be larger than {@code float}'s maximum exponent,
	 * an infinity is returned. Note that if the result is subnormal, precision may be lost; that is, when {@code scalb(x, n)} is subnormal, {@code scalb(scalb(x, n), -n)} may not
	 * equal <i>x</i>. When the result is non-NaN, the result has the same sign as {@code f}.
	 *
	 * <p>
	 * Special cases:
	 * <ul>
	 ...
	 *
	 * <li>If both arguments are signed zeros, {@code direction} is returned unchanged (as implied by the requirement of returning the second argument if the arguments compare as
	 * equal).
	 *
	 * <li>If {@code start} is &plusmn;{@code Double.MIN_VALUE} and {@code direction} has a value such that the result should have a smaller magnitude, then a zero with the same
	 * sign as {@code start} is returned.
	 *
	 * <li>If {@code start} is infinite and {@code direction} has a value such that the result should have a smaller magnitude, {@code Double.MAX_VALUE} with the same sign as
	 * {@code start} is returned.
	 *
	 * <li>If {@code start} is equal to &plusmn; {@code Double.MAX_VALUE} and {@code direction} has a value such that the result should have a larger magnitude, an infinity with
	...
	/**
	 * Returns the floating-point number adjacent to the first argument in the direction of the second argument. If both arguments compare as equal, the second argument is
	 * returned.
	 *
	 * <p>
	 * Special cases:
	 ...
	 * <li>If either argument is a NaN, then NaN is returned.
	 *
	 * <li>If both arguments are signed zeros, a {@code float} zero with the same sign as {@code direction} is returned (as implied by the requirement of returning the second
	 * argument if the arguments compare as equal).
	 *
	 * <li>If {@code start} is &plusmn;{@code Float.MIN_VALUE} and {@code direction} has a value such that the result should have a smaller magnitude, then a zero with the same
	 * sign as {@code start} is returned.
	 *
	 * <li>If {@code start} is infinite and {@code direction} has a value such that the result should have a smaller magnitude, {@code Float.MAX_VALUE} with the same sign as
	 * {@code start} is returned.
	 *
	 * <li>If {@code start} is equal to &plusmn; {@code Float.MAX_VALUE} and {@code direction} has a value such that the result should have a larger magnitude, an infinity with
	 * same sign as {@code start} is returned.
	 * </ul>
	 *
	 * @param start
	...
	/**
	 * Returns the floating-point value adjacent to {@code d} in the direction of positive infinity. This method is semantically equivalent to {@code nextAfter(d,
	...
	/**
	 * Returns the floating-point value adjacent to {@code f} in the direction of positive infinity. This method is semantically equivalent to {@code nextAfter(f,
	 * Double.POSITIVE_INFINITY)}; however, a {@code nextUp} implementation may run faster than its equivalent {@code nextAfter} call.
	 *
	 * <p>
	 * Special Cases:
	...
	// Commented to build on Java 1.6
	//	/**
	//	 * Returns the floating-point value adjacent to {@code d} in the direction of negative infinity. This method is semantically equivalent to {@code nextAfter(d,
	//	 * Double.NEGATIVE_INFINITY)}; however, a {@code nextDown} implementation may run faster than its equivalent {@code nextAfter} call.
	//	 *
	//	 * <p>
	//	 * Special Cases:
	//	 * <ul>
	...
	// Commented to build on Java 1.6
	//	/**
	//	 * Returns the floating-point value adjacent to {@code f} in the direction of negative infinity. This method is semantically equivalent to {@code nextAfter(f,
	//	 * Float.NEGATIVE_INFINITY)}; however, a {@code nextDown} implementation may run faster than its equivalent {@code nextAfter} call.
	//	 *
	//	 * <p>
	//	 * Special Cases:
	//	 * <ul>
	...
	/**
	 * Returns the first floating-point argument with the sign of the second floating-point argument. For this method, a NaN {@code sign} argument is always treated as if it were
	 * positive.
	 *
	 * @param magnitude
	 *            the parameter providing the magnitude of the result
	...
	/**
	 * Returns the size of an ulp of the argument. An ulp of a {@code double} value is the positive distance between this floating-point value and the {@code double} value next
	 * larger in magnitude. Note that for non-NaN <i>x</i>, <code>ulp(-<i>x</i>) == ulp(<i>x</i>)</code>.
	 *
	 * <p>
	 * Special Cases:
	 * <ul>
	...
	/**
	 * Returns the size of an ulp of the argument. An ulp of a {@code float} value is the positive distance between this floating-point value and the {@code float} value next
	 * larger in magnitude. Note that for non-NaN <i>x</i>, <code>ulp(-<i>x</i>) == ulp(<i>x</i>)</code>.
	 *
	 * <p>
	 * Special Cases:
