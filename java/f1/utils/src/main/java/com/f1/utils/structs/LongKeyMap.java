...
public class LongKeyMap<V> implements LongKeyMapSource<V>, ToStringable, Clearable, Cloneable {

	private final class Keys implements LongIterable {
		@Override
	...
	public static final LongKeyMap EMPTY = new LongKeyMap<Object>() {
		@Override
	...
	public LongKeyMap(int bucketsCount) {
		int i = (int) MH.clip(MH.getPowerOfTwoUpper(bucketsCount) << 2, 4, 1 << 30);
		buckets = new Node[i];
		this.threshold = buckets.length >> 1;
		mask = i - 1;
	}
	public LongKeyMap(LongKeyMap<V> other) {
	...
	public V get(Long key) {
		return key == null ? null : get(key.longValue());
	}
	@Override
	public V get(long key) {
	...
	public Node<V> getNodeOrCreate(long key) {
		int h = hash(key);
		Node<V> bucket = buckets[h];
		while (bucket != null)
			if (bucket.key == key)
	...
	public static class Node<V> implements ToStringable, Comparable<Node<V>>, Map.Entry<Long, V> {
	...
	@Override
	public Map<Long, V> fill(Map<Long, V> sink) {
		for (Node<V> n : this)
	...
	public void addAll(LongKeyMap<V> types) {
		for (Node<V> v : types)
			put(v.key, v.value);
	}
	...
	private Node<V> newNode(long key, Object value, Node next) {
		if (poolSize == 0)
	...
	static public StringBuilder diagnose(LongKeyMap<?> map, StringBuilder sink) {
	...
	static public String diagnose(LongKeyMap<?> map) {
	...
	public void putOrThrow(long key, V value) {
		int h = hash(key);
		Node<V> bucket = buckets[h];
		while (bucket != null)
			if (bucket.key == key) {
	...
	public V removeOrThrow(long key) {
		int h = hash(key);
		Node<V> bucket = buckets[h], last = null;
		while (bucket != null)
			if (bucket.key == key) {
	...
	public V updateOrThrow(long key, V value) {
		Node<V> bucket = buckets[hash(key)];
		while (bucket != null)
			if (bucket.key == key) {
				V old = bucket.value;
	...
	@Override
	public LongKeyMap<V> clone() {
		return new LongKeyMap<V>(this);
	}
}
