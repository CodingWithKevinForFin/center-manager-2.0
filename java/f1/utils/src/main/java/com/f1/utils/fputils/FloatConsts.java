...
public class FloatConsts {
	...
	/**
	 * A constant holding the smallest positive normal value of type <code>float</code>, 2<sup>-126</sup>. It is equal to the value returned by
	...
	/**
	 * The exponent the smallest positive <code>float</code> subnormal value would have if it could be normalized. It is equal to the value returned by
	...
	static {
		...
		assert (((SIGN_BIT_MASK | EXP_BIT_MASK | SIGNIF_BIT_MASK) == ~0)
				&& (((SIGN_BIT_MASK & EXP_BIT_MASK) == 0) && ((SIGN_BIT_MASK & SIGNIF_BIT_MASK) == 0) && ((EXP_BIT_MASK & SIGNIF_BIT_MASK) == 0)));
	}
}