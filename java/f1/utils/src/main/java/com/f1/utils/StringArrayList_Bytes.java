 ...
 * 
 */
public class StringArrayList_Bytes implements ToStringable {
	private static final Logger log = LH.get();

	private static int NULL = -1;
	private static int EMPTY = -2;
	public static final int MAX_LENGTH = Byte.MAX_VALUE - 1;
	private int[] pointers = new int[32];//-1 indicates null, otherwise, address of header within data field
	private byte[] data = new byte[1024];//header is single char and indicates length (not including header), neg value indicates length of unused bytes (including header)
	private int nextPointer = 0;
	private int size;
	private int unusedChars = 0;
	private final StringBuilder tmp = new StringBuilder();

	public StringArrayList_Bytes() {
	}
	...
	private int fill(CharSequence cs, int pos) {
		...
		for (int i = 0; i < len; i++) {
			char c = cs.charAt(i);
			if (c > 127)
				throw new RuntimeException("Not ASCII at " + i + ": '" + cs + "'");
			data[pos++] = (byte) c;
		}
		return pos;
	}
	...
	private void defrag(int newArraySize) {
		...
		byte[] tmp = new byte[newArraySize];
		int tmpPointer = 0;
	...
	private static int toSignedInt(byte c) {
		return c;
	}
	...
	public static void main(String a[]) {
		StringArrayList_Bytes sal = new StringArrayList_Bytes();
		for (int i = 0; i < 10 * 1000 * 1000; i++) {
			if (i % 1000000 == 0)
				System.out.println(i);
			sal.add(GuidHelper.getGuid(62).substring(20));
		}
		System.gc();
		OH.sleep(1000);
