...
public class DerivedCellCalculatorSqlUpdate extends DerivedCellCalculatorSql {

	private SqlColumnsNode node;
	private DerivedCellCalculatorSql from;

	public DerivedCellCalculatorSqlUpdate(SqlColumnsNode updateNode, SqlDerivedCellParser dcp, DerivedCellCalculatorSql from) {
	...
	private FlowControl get(CalcFrameStack lcvs, FlowControl o) {
		...
		FlowControl r = getProcessor().getSqlProcessor().getUpdateProcessor().processUpdate(lcvs, this, node, table);
