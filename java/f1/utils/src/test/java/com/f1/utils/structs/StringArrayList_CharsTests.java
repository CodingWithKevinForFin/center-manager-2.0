...
public class StringArrayList_CharsTests {
	...
	public void test1() {
		...
		for (int rand = 0; rand < 17; rand += 17) {
			...
			t.verify();
			for (int n = 0; n < cnt; n++)
				t.add(r.nextInt(t.size()), t.newString(r.nextInt(200)));
			t.verify();
			while (t.size() > 1)
				t.remove(r.nextInt(t.size()));
			t.verify();
			...
			for (int n = 0; n < cnt / 10; n++) {
				...
				for (int x = 0; x < 10; x++) {
					t.set(i, t.newString(r.nextInt(200)));
				}
			}
			t.verify();
			for (int x = 0; x < 10; x++) {
				t.set(0, t.newString(r.nextInt(200)));
			}
			t.verify();
			for (int x = 0; x < 10; x++) {
				t.set(t.size() - 1, t.newString(r.nextInt(200)));
			}
			t.verify();
			...
			t.verify();
			for (int n = 0; n < 20; n++) {
				t.set(r.nextInt(t.size()), t.newString(Short.MAX_VALUE - 1));
			}
			t.verify();
		}
	}
	...
	public static class Tester {
		...
		final StringArrayList_Chars b = new StringArrayList_Chars();
		...
		public String newRandomString() {
			...
			if (n == 31)
				return null;
			return newString(n == 30 ? StringArrayList_Chars.MAX_LENGTH : (n > 20 ? (n * n) : n));
		}
		public void clear() {
			a.clear();
			b.clear();
