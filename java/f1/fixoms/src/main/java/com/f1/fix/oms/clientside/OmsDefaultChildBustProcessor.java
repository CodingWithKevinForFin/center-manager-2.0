 ...
 * 
 */
public class OmsDefaultChildBustProcessor extends OmsAbstractProcessor {
	...
	public void processAction(OrderEventCtx action, OmsOrderState state, ThreadScope threadScope) throws Exception {
		...
		Execution old = order.getExecutions().put(parentExec.getId(), parentExec);
		Order fixOrder = order.getFixOrder();
		if (orig != null && orig.getExecStatus() != 1) {
			orig.setExecStatus(1);//TODO: USE A DAMN EXECSTATUS ENUM
			fixOrder.setTotalExecQty(fixOrder.getTotalExecQty() - orig.getExecQty());
			fixOrder.setTotalExecValue(fixOrder.getTotalExecValue() - orig.getExecPx() * orig.getExecQty());
		}

		ExecType execType = null;

		if (!OmsUtils.isTerminal(order.getOrderStatus()) || OmsUtils.isInState(order.getOrderStatus(), OmsOrderStatus.FILLED)) {
			order.setExchLeaves(order.getExchLeaves() + orig.getExecQty());
		}
		...
		notification.setType(OmsAction.ATTACH_BUST_EXECUTION);
