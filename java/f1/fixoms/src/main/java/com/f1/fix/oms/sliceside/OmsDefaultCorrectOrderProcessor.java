...
public class OmsDefaultCorrectOrderProcessor extends OmsAbstractProcessor {
	...
	public void processAction(OrderEventCtx action, OmsOrderState state, ThreadScope threadScope) throws Exception {
		...
		if (report.getPosResend()) {
			if (CH.isntEmpty(slice.getExecutions()) && slice.getExecutions().containsKey(report.getExecution().getId())) {
				LH.info(log, "Ignoring pos-resend execution correct:", report);
				return;
			}
			LH.info(log, "processing pos-resend execution correct:", report);
		}
		...
		Execution orig = OmsUtils.processCorrect(slice, report, "slice");
		...
		action.setOrderAction(OmsAction.CHILD_EXEC_CORRECT);
