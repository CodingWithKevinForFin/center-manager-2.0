...
public class PriceFormatterTag extends NumberFormatterTag {
	...
	protected void format(JspBuilderSession session, HttpTag tag, int i) {
		...
		session.getBody().append("localFormatter.getPriceFormatter(").append(decimals).append(").format(").append(value).append(", out);");
